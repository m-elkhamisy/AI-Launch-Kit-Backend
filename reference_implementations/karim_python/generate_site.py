...
# Optional: real-world section blueprints mined from HyperUI (MIT-licensed, HTML/Tailwind).
# Drop hyperui_reference.txt next to this script to enable; the model imitates these structures.
try:
    _ref = pathlib.Path(__file__).with_name("hyperui_reference.txt")
    if _ref.exists():
        DESIGN_SYSTEM += ("\n\nREAL SECTION BLUEPRINTS (imitate these proven HTML/Tailwind structures, "
                          "but rewrite copy for THIS brand, apply the chosen palette/fonts, and swap the "
                          "placeholder Unsplash images for the assigned images):\n" + _ref.read_text(encoding="utf-8"))
        print("  design blueprints loaded (HyperUI reference)")
except Exception:
    pass
...
if not answers:  # ---- terminal intake (only when no answers.json) ----
    ...
    answers["theme"]     = ask_choice("8) Theme mode?",
        ["Light mode", "Dark mode", "Light + dark (include a theme toggle)"])
    answers["animation"] = ask_choice("9) Animation level?",
        ["Minimal (subtle & clean)", "Low (light movements)",
         "Balanced (recommended)", "High (more dynamic)"])

    if prefill:
        answers["cta"]   = review("10) Main call-to-action?", prefill.get("cta"))
        answers["extra"] = prefill.get("extra") or "(not specified)"   # taken from the profile silently
    else:
        answers["cta"]   = ask_text("10) Main call-to-action? (e.g. 'Order Now', 'Book a Call')")
        answers["extra"] = "(not specified)"
    ...
    while True:
        ...
        if cmd.lower().startswith("t ") and cmd[2:].strip().isdigit():
            ...
            if num.isdigit() and 0 <= int(num)-1 < len(catalog):
                catalog[int(num)-1]["name"] = new.strip()
...
DIRECTIONS = [
    ("Split editorial", "LIGHT background using the palette. Two-column hero: left = big headline + subline "
     "+ CTAs on a clean light/tinted panel; right = the hero image in a rounded frame. Serif or strong "
     "display heading. NO full-bleed dark photo. Whitespace-forward, refined."),
    ("Bold color-block", "Use the palette's PRIMARY color as a large solid background block behind the hero "
     "text (not a photo). Oversized punchy sans headline in a contrasting color, confident CTAs. The image "
     "is a small framed accent or omitted. High energy, minimal photography."),
    ("Modern overlay", "Full-width hero image with a tint of the palette's primary over it, centered or "
     "left-aligned headline, glassy/rounded CTA buttons using the accent color. Sleek and contemporary."),
    ("Warm organic", "Soft tinted background from the palette, rounded shapes, friendly copy, image in an "
     "organic/rounded frame."),
    ("Clean minimal", "Mostly whitespace, restrained palette use, precise grid, small tasteful image, "
     "understated type."),
]
...
PAGES (build exactly these as separate routed pages, shared sticky nav + footer):
{pages_desc}
...
"""
    ...
    for i in range(120):
        ...
        if g.status_code != 200:
            print(f"  [{i+1}] not ready (HTTP {g.status_code})"); continue
        data = g.json()
        v = data.get("latestVersion") or {}
        # status can live in a few places depending on API version
        st = (v.get("status") or data.get("status")
              or (data.get("latestVersion") or {}).get("state"))
        has_output = bool(v.get("demoUrl") or v.get("files") or v.get("id"))
        print(f"  [{i+1}] status: {st}{'  (output ready)' if has_output and not st else ''}")
        if st in ("completed", "ready", "done") or (has_output and st not in ("pending", "generating", "in_progress")):
            version_id = v.get("id") or data.get("latestVersionId")
            if v.get("demoUrl"):
                print("  instant preview:", v.get("demoUrl"))
            if version_id:
                break
        if st in ("failed", "error"):
            print("v0 generation failed."); raise SystemExit
    if not version_id:
        print("Timed out (no version id). Open the edit link above to check the chat.")
        raise SystemExit
    ...
    if not imgs:
        ...
        for fp in imgs:
            ...
            if "logo" in label.lower() and not logo_path:
                logo_path = rel
                print(f"    ^ using this as the site favicon")
