...
|---|---|---|---|---|---|
| `launchkit.domain.models.*` | Define cohesive Pydantic models grouped into intake, design, planning, generation, profile/image, guardrail, storage, and deployment modules. | Request-independent domain models only; no HTTP bodies or provider SDK objects. | TS `types/*`, Python route/pipeline result shapes, Karim dict shapes. | none | Pure models; sync; unit tests for validation/defaults/aliases; exact field compatibility where types are known, semantic for informal Python shapes; initial. |
| `launchkit.application.intake.normalization` | Normalize raw submission mappings: `flatten_submission(raw)`, `normalize_company(raw)`, `missing_required_fields(form)`. | raw mapping, `LegacyCompany`, `OnboardingForm`. | `local_store.py`, `s3_store.py`, `lib/form-config.ts`, `types/form.ts`. | `launchkit.domain.models.intake` | Pure business logic; sync; table-driven tests for aliases/nested raw; exact compatibility for existing aliases; initial. |
