...
| `launchkit.storage.contracts` | `SubmissionStore` protocol: save, normalized/raw reads, and sorted listing. | stored/raw records and IDs. | duplicated local/S3 function shapes. | storage models | Synchronous boundary with structural contract tests; migrated in stage 9. |
| `launchkit.storage.local` | Atomic JSON-file storage adapter using the legacy raw/normalized layout. | storage records. | `local_store.py`. | storage contract, intake normalization. | Sync filesystem I/O with temp-directory tests; migrated in stage 9. |
| `launchkit.storage.s3` | Injected-client S3 storage adapter using the same object layout. | storage records. | `s3_store.py`. | storage contract, intake normalization, boto3. | Sync SDK boundary with fake-client tests and no import-time client; migrated in stage 9. |
...
| `launchkit.generation.mockups` | Generate three design directions through `MockupGenerationService.generate(...)` with deterministic fallback HTML. | mockup result/designs. | `site-pipeline.ts`, Karim mockup flow. | brief, prompts, HTML generator, image sourcing. | Async staged coordination with fallback and escaping tests; migrated before stage 11 orchestration. |
| `launchkit.planning.normalization` / `service` | Normalize model JSON with `parse_site_plan` and generate/revise plans through `SitePlanningService.generate`. | site plan. | `generateSitePlan` in `site-pipeline.ts`, `plan-text.ts`, Karim plan logic. | brief, plan prompt, text generator, planning models/text | Pure transforms plus async provider coordination; invalid JSON, revisions, home fallback, defaults, and duplicate slugs tested; migrated in stage 4/11. |
...
| `launchkit.images.sourcing` | Resolve planned image specs from uploaded, AI, Pexels, or placeholder sources: `source_images_for_page`, `render_image_catalog`. | sourced and planned images. | `image-sourcing.ts`, Karim image generation. | image generator/search contracts, Pexels adapter. | Async graceful-degradation coordination; mocked providers; migrated in stage 7. |
| `launchkit.images.registry` | Replace large data URIs with prompt-safe tokens: `ImageRegistry.compress`, `ImageRegistry.resolve`. | strings/data URLs. | `image-sourcing.ts`. | none | Pure sync behavior with stable token reuse tests; migrated in stage 7. |
...
| `launchkit.adapters.v0` | v0 create/init/status/download/handoff through `V0Adapter`. | generation, handoff, and archive results. | `v0.ts`, `pipeline.py` v0 functions. | HTTPX | Async external adapter with mocked status/404/402/privacy/download tests; migrated in stage 8. |
| `launchkit.deployment.claim` | Coordinate ownership transfer through `ClaimDeploymentService.deploy(chat_id)`. | deployment files, provider deployment, final claim result. | `vercel_deploy.py`. | v0 archive and Vercel gateway contracts | Async capability coordination with ZIP and fake-gateway tests; migrated in stage 10. |
| `launchkit.archive` | Create ZIP or single HTML download payloads: `build_site_zip`, `html_download_payload`. | built pages and archive bytes. | Haseeb API `site/zip`, `download-html`, Karim zip creation. | zipfile | Pure sync construction with archive-content tests; migrated in stage 8. |
| `launchkit.generation.page_builder` | Build the homepage shell then secondary pages through `PageBuildService.build(...)`. | built pages and warnings. | `buildAllPages` in `site-pipeline.ts`, Karim page loop. | prompts, HTML generation/processing, image sourcing. | Async with secondary concurrency two, fatal home errors, warning-isolated secondary errors; migrated before stage 11 orchestration. |
| `launchkit.generation.service` | Coordinate capabilities only through `WebsiteGenerationService.generate(request)`. | generation request/result. | `runBuild` in `site-pipeline.ts`, `pipeline.py`, Karim procedural flow. | brief, page builder, copy extractor, image catalogs, v0 gateway | Async orchestration with offline Claude/v0/both integration tests and warning/failure policies; migrated in stage 11. |
