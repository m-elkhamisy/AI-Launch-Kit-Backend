...
| `launchkit.profiles.extraction` | Extract text/images and fields from uploaded profile files: `extract_profile(bytes, filename, extractor, image_labeler)`. | profile extraction result, extracted images. | `lib/profile-extraction.ts`, Karim file/image helpers. | profile/intake models, LLM adapter. | Mixed pure/external; async preferred for LLM labeling; tests with small fixture files and mocked LLM; semantic compatibility; initial after models/prompts. |
...
| `launchkit.adapters.llm_queue` | Shared request throttle: `RequestQueue.run`, `note_rate_limit`. | queue config. | `request-queue.ts`. | asyncio/time | External coordination; async; timing-light tests with fake clock where practical; semantic compatibility; initial before provider adapters. |
| `launchkit.adapters.openrouter` | OpenRouter text/JSON/image adapter: `generate_text`, `generate_json`, `generate_image`, `label_image`. | normalized capability results/errors. | `openrouter.ts`, `anthropic.ts`, `openrouter-actions.ts`, Python direct requests. | queue, HTTP client | External adapter; async preferred; mocked HTTP tests; semantic compatibility; initial after queue. |
